...
async def upsert_profile(payload: ProfileIn, db=Depends(get_session)):
    user = (await db.execute(select(User).where(User.email == payload.user_email))).scalars().first()
    if not user:
        user = User(email=payload.user_email, name=payload.name)
        db.add(user)
        await db.flush()

    profile = (await db.execute(select(Profile).where(Profile.user_id == user.id))).scalars().first()
    if not profile:
    ...
    await db.execute(Experience.__table__.delete().where(Experience.profile_id == profile.id))
    await db.execute(Education.__table__.delete().where(Education.profile_id == profile.id))
