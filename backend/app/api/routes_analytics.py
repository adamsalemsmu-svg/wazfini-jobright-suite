...
def get_user_summary(user_id: int, db: Session = Depends(get_db)):
    ...
    success = (
        db.query(Metric)
        .filter(Metric.user_id == user_id, Metric.status == "SUCCESS")
        .count()
    )
    failure = (
        db.query(Metric)
        .filter(Metric.user_id == user_id, Metric.status == "FAILURE")
        .count()
    )
    avg_duration = (
        db.query(func.avg(Metric.duration)).filter(Metric.user_id == user_id).scalar()
        or 0
    )
    return {
        "user_id": user_id,
